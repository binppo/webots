...
#define __OPC_TREEBUILDERS_H__
	...
	class OPCODE_API AABBTreeOfVerticesBuilder : public AABBTreeBuilder
	{
		...
		_override(AABBTreeBuilder)	bool			ComputeGlobalBox(const dTriIndex* primitives, udword nb_prims, AABB& global_box)	const;
		_override(AABBTreeBuilder)	float			GetSplittingValue(udword index, udword axis)									const;
		_override(AABBTreeBuilder)	Point			GetSplittingValues(udword index) const;
		_override(AABBTreeBuilder)	float			GetSplittingValue(const dTriIndex* primitives, udword nb_prims, const AABB& global_box, udword axis)	const;
