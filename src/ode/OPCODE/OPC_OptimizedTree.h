...
#define __OPC_OPTIMIZEDTREE_H__
	...
	#define IMPLEMENT_COLLISION_TREE(base_class, node)																\
		...
		_override(AABBOptimizedTree)	bool			Build(AABBTree* tree) override;											\
		/* Refits the tree */																						\
		_override(AABBOptimizedTree)	bool			Refit(const MeshInterface* mesh_interface) override;						\
		/* Walks the tree */																						\
		_override(AABBOptimizedTree)	bool			Walk(GenericWalkingCallback callback, void* user_data) const override;	\
		/* Data access */																							\
		inline_						const node*		GetNodes()		const	{ return mNodes;					}	\
		/* Stats */																									\
		_override(AABBOptimizedTree)	udword			GetUsedBytes()	const override	{ return mNbNodes*sizeof(node);		}	\
		private:																									\
									node*			mNodes;
