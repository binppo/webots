...
struct dxGeom : public dBase {
    ...
    dFluidID fluid;		// dynamics fluid associated with this object (if any)
    dxGeom *body_next;	// next geom in body's linked list of associated geoms
    dxGeom *fluid_next;	// next geom in fluid's linked list of associated geoms
    ...
    bool tag;
    bool is_dynamic;   // used by ODE_MT to flag the geom as dynamic (it is clustered by the clustering algorithm)
    ...
    dxGeom *parent_geom; // parent space only if embedded into a transform geom class
    dxSpace *parent_space;// the space this geom is contained in, 0 if none
    dxSpace *old_space;   // the space this geom was contained in originally, 0 if none. Used for clustering
    ...
    // add and remove this geom from a linked list maintained by a fluid.

    void fluidAdd (dxFluid *f) {
      fluid = f;
      fluid_next = f->geom;
      f->geom = this;
    }
